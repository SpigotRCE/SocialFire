...
public class Config<T, P> extends ConfigProvider {
  ...
  public Map<String, LinkModel> getLinks() {
    ...
    getFileConfig().getSection(Route.fromString("announcements")).getKeys().forEach(type -> {
      Map<String, String> fields = new HashMap<>();
      getFileConfig().getSection(Route.fromString("announcements." + type))
        .getKeys()
        .forEach(field -> fields.put((String) field,
          getFileConfig().getString(Route.fromString("announcements." + type + "." + field))));
      models.put((String) type, new LinkModel().deserialize(fields));
    });
