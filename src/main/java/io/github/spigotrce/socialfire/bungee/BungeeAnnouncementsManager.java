...
public class BungeeAnnouncementsManager extends AbstractAnnouncementsManager<ScheduledTask, ProxiedPlayer> {
    ...
    public void reload() {
        ...
        ANNOUNCEMENTS.forEach((name, model) -> {
            ...
            ScheduledTask task = BungeeFire.PROXY_SERVER.getScheduler().schedule(
                    BungeeFire.INSTANCE,
                    () -> BungeeFire.PROXY_SERVER.getPlayers()
                            .forEach(
    ...
    @Override
    public void sendAnnouncement(ProxiedPlayer player, LinkModel model) {
