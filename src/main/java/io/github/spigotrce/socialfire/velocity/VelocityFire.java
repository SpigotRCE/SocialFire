...
@Plugin(id = "socialfire", name = "SocialFire", version = "1.4", authors = "SpigotRCE") public class VelocityFire {
  public static VelocityFire INSTANCE;

  @Inject public static Logger LOGGER;
  @Inject @DataDirectory public static Path DATA_DIRECTORY;
  @Inject public static ProxyServer PROXY_SERVER;
  ...
  @Inject
  public VelocityFire(Logger logger,
  ...
  public void reloadCommands() {
    ...
    PROXY_SERVER.getCommandManager()
      .register(PROXY_SERVER.getCommandManager()
        .metaBuilder("socialfire:callback")
        .aliases(new ArrayList<>(CONFIG.getLinks().keySet()).toArray(new String[0]))
        .build(), new DummyCommand());
  ...
  @Subscribe public void onTabComplete(TabCompleteEvent event) {
  ...
  @Subscribe public void onCommand(CommandExecuteEvent event) {
    CONFIG.getLinks().keySet().forEach(label -> {
  ...
  @Subscribe public void onAvailableCommands(PlayerAvailableCommandsEvent event) {
    event.getRootNode().removeChildByName("socialfire:callback");
  }

  @Subscribe public void onPluginMessage(PluginMessageEvent event) {
    if (!Objects.equals(event.getIdentifier(), CHANNEL_NAME)) {
      return;
    }
